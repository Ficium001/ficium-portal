...
function useConn(): Conn {
  ...
  useEffect(() => {
    ...
    const ping = async () => {
      try {
        await adminDb.rpc('detect_portal_user_type', {
...
function IdleWarning({ onStay, onLeave }: { onStay: () => void; onLeave: () => void }) {
  return (
    <div className='fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4'
      ...
      <div className='bg-ink/80 border border-amber-800 rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl'>
        ...
        <p className='text-ink/60 text-[12px] mb-6'>
        ...
        <div className='flex gap-3'>
          ...
          <button onClick={onLeave}
            className='flex-1 border border-ficium/[0.25] text-ink/60 font-semibold py-2.5 rounded-xl text-[13px] hover:bg-ficium/[0.04]'>
            Sign out
          </button>
        </div>
      </div>
...
function StatusBar({
  ...
  return (
    <div className='h-6 bg-ink/95 border-t border-ficium/[0.12] flex items-center px-4 gap-4 flex-shrink-0 text-[9px] font-mono'
      ...
      <span className='text-[#2d3748]'>·</span>
      <span className='text-ink/20'>{adminId.slice(0, 8)}</span>
      {pendingDc > 0 && (
        <><span className='text-[#2d3748]'>·</span>
      ...
      <span className='ml-auto text-ink/20'>G+D Dashboard · G+U Users · G+Q Dual Control · G+L Audit</span>
    </div>
  )
}
...
export default function AdminPortalShell() {
  ...
  return (
    <div className='flex flex-col h-screen bg-ink text-ink/90 overflow-hidden'>
      {warning && <IdleWarning onStay={reset} onLeave={signOut} />}

      <div className='flex flex-1 overflow-hidden'>
        {/* Sidebar */}
        <aside className='w-56 bg-ink/95 border-r border-ficium/[0.12] flex flex-col flex-shrink-0' aria-label='Admin navigation'>
          {/* Logo */}
          <div className='px-4 py-4 border-b border-ficium/[0.12] flex items-center gap-3'>
          ...
          <nav className='flex-1 py-3 overflow-y-auto' aria-label='Primary navigation'>
            <p className='text-[8px] font-bold text-ink/20 uppercase tracking-[0.15em] px-4 mb-2'>Navigation</p>
            {visible.map(item => (
              <NavLink
                key={item.section}
                to={item.path}
                ...
                className={({ isActive }) => [
                  ...
                  isActive
                    ...
                    : 'text-ink/45 hover:text-ink/90 hover:bg-ficium/[0.04]',
                ].join(' ')}
          ...
          {/* Footer */}
          <div className='border-t border-ficium/[0.12] p-3'>
            <div className='flex items-center gap-2.5 mb-2.5 px-1'>
              ...
              <div className='min-w-0'>
                <div className='text-[11px] font-semibold text-ink/75 truncate'>{me?.display_name ?? 'Admin'}</div>
                <div className='text-[9px] text-ink/30 truncate font-mono'>{me?.role_slug ?? '—'}</div>
              </div>
            </div>
            <button onClick={signOut}
              className='flex items-center gap-2 text-[11px] text-ink/30 hover:text-red-400 transition-colors w-full px-1'
              aria-label='Sign out'>
        ...
        <div className='flex-1 flex flex-col overflow-hidden'>
          {/* Top bar */}
          <header className='h-12 bg-ink/95 border-b border-ficium/[0.12] flex items-center justify-between px-5 flex-shrink-0'>
            <div className='flex items-center gap-1.5 text-[12px] text-ink/45'>
              ...
              <ChevronRight className='w-3 h-3 text-ink/20' aria-hidden />
              <span className='text-ink/75 font-medium'>{me?.display_name ?? 'Admin'}</span>
            </div>
            <div className='flex items-center gap-3'>
              ...
              <button
                className='relative w-8 h-8 rounded-xl hover:bg-ficium/[0.04] flex items-center justify-center transition-colors text-ink/45 hover:text-ink/90'
