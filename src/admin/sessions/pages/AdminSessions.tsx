        ...
        <ADataTable headers={['User', 'Role', 'IP address', 'Location', 'Started', 'Last active', 'Duration', 'Status', '']} caption='Admin sessions'>
          {sessions.map(s => {
            ...
            return (
              <ATr key={s.id}>
                <ATd>
                  <div className='font-semibold text-[12px] text-slate-200'>{s.admin_email ?? '—'}</div>
                  <AMonoRef value={s.id} />
                  {isSelf && <span className='text-[9px] text-indigo-400 font-bold block'>CURRENT SESSION</span>}
                </ATd>
                <ATd className='text-[11px] font-mono text-indigo-300'>{s.admin_role ?? '—'}</ATd>
