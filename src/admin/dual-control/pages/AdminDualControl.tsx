    ...
    <tr className='bg-[#090c13]'>
      <td colSpan={7} className='px-5 py-5'>
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-5'>
          ...
          {/* Payload */}
          <div>
            <div className='text-[9px] font-bold text-white/30 uppercase tracking-widest mb-2.5'>Action payload</div>
            <pre className='text-[10px] font-mono text-white/60 bg-[#0d1117] border border-[#1f2937] rounded-lg p-3 overflow-auto max-h-52 leading-relaxed'>
              {JSON.stringify(action.payload, null, 2)}
            </pre>
            {action.payload_before && (
              <>
                <div className='text-[9px] font-bold text-white/30 uppercase tracking-widest mb-2.5 mt-4'>Previous state</div>
                <pre className='text-[10px] font-mono text-white/45 bg-[#0d1117] border border-[#1f2937] rounded-lg p-3 overflow-auto max-h-32 leading-relaxed'>
          ...
          {/* Maker + timeline */}
          <div>
            <div className='text-[9px] font-bold text-white/30 uppercase tracking-widest mb-2.5'>Provenance</div>
            <div className='space-y-2 text-[11px]'>
              {[
                ...
                <div key={l} className='flex justify-between'>
                  ...
                  <span className='font-mono text-white/60 max-w-[55%] text-right truncate'>{v}</span>
                </div>
              ))}
            </div>
            {action.checker_id && (
              <div className='mt-4 bg-[#0d1117] border border-[#1f2937] rounded-lg p-3 text-[11px]'>
                <div className='text-[9px] font-bold text-white/30 uppercase tracking-widest mb-2'>Checker decision</div>
                <div className='space-y-1'>
                  <div className='flex justify-between'><span className='text-white/30'>Checker</span><span className='font-mono text-white/60'>{action.checker_email}</span></div>
                  <div className='flex justify-between'><span className='text-white/30'>Role</span><span className='font-mono text-white/60'>{action.checker_role}</span></div>
                  <div className='flex justify-between'><span className='text-white/30'>IP</span><span className='font-mono text-white/60'>{action.checker_ip}</span></div>
                  <div className='flex justify-between'><span className='text-white/30'>At</span><span className='font-mono text-white/60'>{action.checked_at ? new Date(action.checked_at).toLocaleString('en-MU') : '—'}</span></div>
                  {action.checker_note && <div className='text-white/45 italic mt-1'>"{action.checker_note}"</div>}
                </div>
              </div>
            )}
          </div>
          ...
          {/* Action buttons */}
          <div>
            <div className='text-[9px] font-bold text-white/30 uppercase tracking-widest mb-2.5'>Decision</div>
...
function AFormField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className='block text-[10px] font-bold text-white/45 uppercase tracking-widest mb-1.5'>{label}</label>
      {children}
    </div>
  )
}
...
export default function AdminDualControl() {
  ...
  return (
    <main className='p-6 lg:p-8 max-w-[1440px] mx-auto'>
      ...
      <div className='bg-[#0d1117] border border-ficium/20/50 rounded-xl px-5 py-3 flex items-center gap-3 mb-5'>
              ...
              <>
                <ATr key={a.id} selected={isOpen} onClick={() => setExpanded(isOpen ? null : a.id)}>
                  <ATd>
                    ...
                    <div className='text-[9px] text-white/30 mt-0.5'>{a.action_category}</div>
                  </ATd>
                  <ATd><RiskBadge risk={a.risk} /></ATd>
                  <ATd>
                    <div className='text-[11px] font-mono text-white/60'>{a.maker_email}</div>
                    <div className='text-[9px] text-white/30 font-mono'>{a.maker_ip}</div>
